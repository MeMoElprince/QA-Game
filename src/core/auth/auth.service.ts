...
export class AuthService {
    ...
    async sendTokenEmailVerification(user: User): Promise<{ message: string }> {
        ...
        if (this.config.get('NODE_ENV') === Environment.PRODUCTION)
            url = `${this.config.get('BASE_URL')}/api/auth/verify-account?token=${token}`;
        else
            url = `${this.config.get('BASE_URL_DEV')}/api/auth/verify-account?token=${token}`;
    ...
    async googleSignIn(googleLoginDto: GoogleLoginDto, ip: string) {
        ...
        if (!user.googleId) {
            updatedValues.googleId = googleUserId;
        }
